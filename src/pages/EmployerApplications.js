...
// import { Briefcase, User, Mail, Clock, ChevronDown, Check, X, Clock as PendingIcon } from 'lucide-react';
...
//       <h1 className="text-3xl font-bold text-gray-800 mb-8">Applications Management</h1>
...
//           {jobs.map(job => (
//             <div key={job._id} className="bg-white shadow rounded-lg overflow-hidden">
//               <div 
//                 className="px-6 py-4 border-b cursor-pointer flex justify-between items-center"
...
//                     <div key={applicant._id} className="px-6 py-4 flex items-center justify-between">
...
//                             <Mail className="w-4 h-4 mr-1" /> {applicant.email}
//                           </p>
//                         </div>
//                       </div>
...
//                               }}
//                               className="appearance-none bg-gray-100 border border-gray-300 rounded-md py-1 pl-2 pr-8 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
//                             >
...
useEffect(() => {
  ...
  const handleStatusUpdate = async (jobId, applicantId, status) => {
    try {
      ...
      setJobs(prevJobs => prevJobs.map(job => {
        if (job._id === jobId) {
          const revertedApplicants = job.applicants.map(applicant => {
            if (applicant._id === applicantId) {
              return { ...applicant, status: applicant.status }; // Revert to previous status
            }
            return applicant;
          });
          return { ...job, applicants: revertedApplicants };
        }
        return job;
      }));
